...
public class Blur {
  ...
  private static void blurFile(String filename, int numIterations) {
    ...
    String blurname = "boxblur_" + filename;
  ...
  private static void GaussianFilterFile(String filename, int numIterations) {
	    System.out.println("Reading image file " + filename);
	    PixImage image = ImageUtils.readTIFFPix(filename);

	    System.out.println("Blurring image file.");
	    PixImage blurred = image.GaussianFilter(numIterations);   
	    
	    String blurname = "GaussianFilterblur_" + filename;
	    System.out.println("Writing blurred image file " + blurname);
	    TIFFEncoder.writeTIFF(blurred, blurname);
	    /*
	    TIFFEncoder.writeTIFF(new RunLengthEncoding(edges), "rle" + blurname);
  ...
  public static void main(String[] args) {
	  //args[0]="woman";
	  //args[1]="6";
	  //blurFile("woman.tiff", 6);//debugging
	  //blurFile("woman.tiff", 1);
	  //blurFile("woman.tiff", 10);
	  //blurFile("woman.tiff", 100);
	  //blurFile("woman.tiff", 1000);
	  
	  //GaussianFilterFile("woman.tiff", 1);//debugging
	  		  //args=new String[2];
	          //args[0]="engine.tiff";
			  //args[1]="5";
    ...
    System.out.println("\nGaussianFilteringImage:\n");
    GaussianFilterFile(args[0], numIterations);
    
    //blurFile("woman.tiff", 6);
    //debugging
  }
}
