...
public class Sobel {
  ...
  private static void sobelFile(String filename, int numIterations,
    ...
    String edgename = "sobelEdge_" + filename;
  ...
  private static void PrewittMaskFile(String filename, int numIterations,
          boolean rle) {
System.out.println("Reading image file " + filename);
PixImage image = ImageUtils.readTIFFPix(filename);
PixImage blurred = image;
...
System.out.println("Performing Sobel edge detection on image file.");
PixImage sobeled = blurred.PrewittMask();

String edgename = "PrewittMaskedge_" + filename;
System.out.println("Writing grayscale-edge image file " + edgename);
TIFFEncoder.writeTIFF(sobeled, edgename);
if (rle) {
String rlename = "rle_" + filename;
...
ImageUtils.displayTIFFs(new PixImage[] { image, sobeled });
  ...
  public static void main(String[] args) {
    ...
    if (args.length >= 2) {
      ...
      //sobelFile("woman.tiff", 0,false);
	  //PrewittMaskFile("woman.tiff", 0,false);
    System.out.println("\nSobelEdgeDetectionImage:\n");
    sobelFile(args[0], numIterations, false);
    System.out.println("\nPrewittMasksFilteringImage:\n");
    PrewittMaskFile(args[0], numIterations, false);
  }
}
